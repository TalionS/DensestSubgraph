...
//
//    int cur = 0;
//    if (x > *std::max_element(degrees_[cur].begin(), degrees_[cur].end())) {
//        return;
//    }
//    for (ui i = bin_[cur][x]; i < n; i++) {
//        VertexID u = vert_[cur][i];
//        auto adj = graph.getOutNeighbors(u);
//        std::sort(adj.begin(), adj.end(),
//                  [&](const int &a, const int &b) -> bool {
//                      return degrees_[1 - cur][a] > degrees_[1 - cur][b];
//                  });
//        VertexID v = adj[std::max(int(x) - 1, 0)];
//        if (degrees_[1 - cur][v] < y) continue;
//        for (auto t: adj) {
//            if (degrees_[1 - cur][t] < y)
//                break;
...
//    }
    ...
    while(degrees_copy[0][vert_copy[0][i]] < x || degrees_copy[1][vert_copy[1][j]] < y) {
        for (; degrees_copy[0][vert_copy[0][i]] < x; i++) {
...
//                    --deg[1][v];
        ...
        for (; degrees_copy[1][vert_copy[1][j]] < y; j++) {
...
inline void XYCore::decDeg(ui cur, VertexID t, std::vector<std::vector<VertexID>> &vert_copy, std::vector<std::vector<ui>> &bin_copy, std::vector<std::vector<ui>> &pos_copy,
...
ui XYCore::skyline_core_num(Graph &graph, ui cur, ui x, ui y, bool reduced) {
    ...
    if (y > std::upper_bound(bin_copy[1 - cur].begin(), bin_copy[1 - cur].end(), vertices_count - x) - bin_copy[1 - cur].begin() - 1)
        return 0;
