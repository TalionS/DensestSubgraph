...
void LinearProgramming::FistaIterate(double learning_rate, double t, double ratio) {
    if (is_directed_) {
        ...
        alpha_new = beta;
        for(ui i = 0; i < edges_count_; i++){
            beta[i].weight_first = alpha_new[i].weight_first + (alpha_new[i].weight_first - alpha[i].weight_first) * (t - 1) / (t + 2);
            beta[i].weight_second = alpha_new[i].weight_second + (alpha_new[i].weight_second - alpha[i].weight_second) * (t - 1) / (t + 2);
        }
        alpha = alpha_new;
        for(ui i = 0; i < nodes_count_; i++){
            r[0][i] = 0;
